...
from sample_receiving_app.models.exceptions import TokenNotFound
from sample_receiving_app.models.blacklist_tokens import BlacklistToken
from sample_receiving_app import db
...
def revoke_token(token_id, user):
    ...
    """
    try:
        token = BlacklistToken.query.filter_by(id=token_id, user_identity=user).one()
        token.revoked = True
        db.session.commit()
    except NoResultFound:
